...
export default async function DashboardPage() {
  ...
  const totalWeight = wWeights.reduce((a: number, b: number) => a + b, 0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {([
          ...
          <div key={s.label} className="bg-[#fefce8] rounded-2xl border border-amber-400 p-5 shadow-sm">
            <div className="w-7 h-7 rounded-lg bg-amber-100 border border-amber-300 flex items-center justify-center mb-3">
      ...
      <div className="bg-[#fefce8] border border-amber-400 rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-4 shrink-0">
          <div className="relative w-16 h-16 shrink-0">
            ...
            <span className="absolute inset-0 flex items-center justify-center text-sm font-black text-[#713f12]">{retirementScore}</span>
          </div>
          <div>
            <p className="text-[10px] text-[#b45309] font-semibold uppercase tracking-wider">Retirement Readiness</p>
            <p className="text-lg font-black leading-tight" style={{ color: scoreColor }}>{scoreLabel}</p>
            <p className="text-xs text-[#b45309] font-light">Target: retire at {targetRetire}</p>
          </div>
        </div>
        <div className="hidden sm:block w-px self-stretch bg-amber-300" />
        <div className="grid grid-cols-3 gap-3 flex-1 text-center">
          <div>
            <p className="text-[10px] text-[#b45309] uppercase tracking-wide font-semibold"><FinTip term="SIP">Invest %</FinTip></p>
            <p className="text-base font-black text-[#713f12] mt-0.5">{investPct.toFixed(0)}%</p>
            <p className="text-[10px] text-[#b45309]">of income</p>
          </div>
          <div>
            <p className="text-[10px] text-[#b45309] uppercase tracking-wide font-semibold"><FinTip term="corpus">Corpus gap</FinTip></p>
            <p className="text-base font-black text-[#713f12] mt-0.5">{fmtCrore(Math.max(fireCorpus - projCorpus, 0))}</p>
            <p className="text-[10px] text-[#b45309]">needed by {targetRetire}</p>
          </div>
          <div>
            <p className="text-[10px] text-[#b45309] uppercase tracking-wide font-semibold"><FinTip term="FIRE">FIRE delay</FinTip></p>
            ...
            <p className="text-[10px] text-[#b45309]">from unnecessary spends</p>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-5 gap-4">
        <div className="md:col-span-3 bg-[#fefce8] rounded-2xl border border-amber-400 p-5 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            {budget > 0 && (
              <span className="text-xs text-[#b45309] bg-amber-100 px-2 py-1 rounded-lg font-medium">{budgetPct.toFixed(0)}% used</span>
            )}
          </div>
          {noSpends ? (
