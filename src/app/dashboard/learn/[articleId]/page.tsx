...
export default async function LearnArticlePage({ params }: LearnArticlePageProps) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-2xl border border-amber-300 bg-background overflow-hidden">
        <div className="px-6 py-5 border-b border-amber-200 bg-surface">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border ${LEVEL_STYLE[article.level]}`}>
              {article.level}
            </span>
            <span className="text-[10px] font-semibold px-2 py-0.5 rounded border border-amber-300 bg-amber-50 text-text2">
              {article.category}
            </span>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-text leading-tight">{article.title}</h1>
          <p className="text-sm sm:text-base text-text2 mt-1.5 leading-relaxed">{article.subtitle}</p>
        </div>
        ...
        <div className="px-6 py-6 space-y-6">
          {article.content.map((section, index) => (
            <article key={`${article.id}-${index}`} className="space-y-2">
              {section.heading && (
                <h2 className="text-base font-bold text-text pb-1 border-b border-amber-200">
                  {section.heading}
                </h2>
              )}
          ...
          <div className="rounded-xl border border-amber-300 overflow-hidden mt-4">
            ...
            <div className="px-4 py-3 bg-surface">
              <p className="text-sm text-text leading-relaxed">{article.applyText}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="flex justify-between items-center gap-3">
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-xl border border-amber-400 bg-surface px-3 py-2 text-xs font-semibold text-text hover:border-amber-500 transition-colors"
        >
