    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          "w-14 h-14",
          "bg-[#713f12] text-[#fefce8] hover:bg-[#92400e]"
        )}
        aria-label="Open Finz AI"
      >
        {open ? <X size={22} /> : <Sparkles size={22} />}
        {!open && (
          <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-amber-400 rounded-full border-2 border-[#fefce8] animate-pulse" />
        )}
      </button>
      ...
      {open && (
        <div className={cn(
          ...
          {/* Header */}
          <div className="flex items-center gap-3 px-5 py-4 bg-[#713f12] shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[#fefce8] font-bold text-base leading-tight">Finz</p>
              <p className="text-amber-300/80 text-xs font-light">Knows your spending · Powered by AI</p>
            </div>
            <button
              onClick={() => setOpen(false)}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-amber-300 hover:text-[#fefce8] hover:bg-white/10 transition-all"
            >
              <X size={16} />
            </button>
          </div>
            ...
            {messages.map((m, i) => (
              <div key={i} className={cn("flex items-end gap-2", m.role === "user" ? "justify-end" : "justify-start")}>
                ...
                <div className={cn(
                  ...
                  m.role === "user"
                    ? "bg-[#713f12] text-[#fefce8] rounded-br-sm"
                    : "bg-white border border-amber-200 text-[#713f12] rounded-bl-sm shadow-sm"
                )}>
                  {m.content}
                </div>
              </div>
            ...
            <div className="px-4 pb-3 flex flex-wrap gap-2 shrink-0">
              {STARTERS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="text-xs px-3 py-2 rounded-xl bg-[#fef9c3] border border-amber-400 text-[#713f12] hover:bg-amber-100 hover:border-amber-400 transition-all font-medium"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="flex items-center gap-2.5 px-4 py-3.5 border-t-2 border-amber-200 bg-[#fefce8] shrink-0">
            <input
              ...
              className="flex-1 bg-[#fef9c3] border border-amber-400 rounded-xl px-4 py-2.5 text-sm text-[#713f12] placeholder-[#b45309] focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-400/20 transition-all"
            />
            <button
              onClick={() => send()}
              disabled={!input.trim() || loading}
              className="w-10 h-10 rounded-xl bg-[#713f12] text-[#fefce8] flex items-center justify-center hover:bg-[#92400e] active:scale-95 transition-all disabled:opacity-40 shrink-0"
            >
              <Send size={15} />
            </button>
          </div>
