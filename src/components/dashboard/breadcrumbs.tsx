...
export function DashboardBreadcrumbs() {
  ...
  return (
    <nav className="mb-2 flex items-center gap-1.5 text-xs text-[#b45309]" aria-label="Breadcrumb">
      <Link href="/dashboard" className="font-medium hover:text-[#713f12] transition-colors">
        Dashboard
      </Link>
        ...
        return (
          <div key={`${segment}-${index}`} className="contents">
            ...
            {isLast || segment === "overview" ? (
              <span className="font-semibold text-[#713f12] truncate">{label}</span>
            ) : (
              <Link href={href} className="font-medium hover:text-[#713f12] transition-colors truncate">
                {label}
              </Link>
            )}
          </div>
